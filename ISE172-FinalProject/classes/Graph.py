#The purpose of the graph class is to create the graph of verticies and edges, 
#standardize the size of the various imported objects
#and use Dijktra's algorithm to establish the shortest path to a random destination.
...
class Graph:
    ...
    def __init__(self, V, E): 
        ...
        for key in self.vertDict:
            x1 = self.vertDict[key][0] * 700 # scaling the map to fit
            y1 = self.vertDict[key][1] * 700 
        ...
        for edge in self.edges:
            ...
            for x in range(0, len(self.edges[edge][1])):
                x1 = self.edges[edge][1][x][0] * 700 # scaling the map to fit
                y1 = self.edges[edge][1][x][1] * 700 
                input = [x1, y1]
                pos.append(input)
