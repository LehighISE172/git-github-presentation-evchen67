#The purpose of the world class is to create the aesthetics for the graph,
#add the idle cars randomly onto the map,
#create the clock and money counter 
#allow the cars to move along the designated path
#and close the program after 30 minutes or when the simulation is closed.
...
class World(AbstractWorld):
	...
	def runSimulation(self, fps=1, initialTime=0, finalTime=30*60):
		...
		#Setting vehicle starting locations and add to list of carNodes
		...
		for t in range(initialTime,finalTime):	
			print ("\n\n Time: %02d:%02d"%(t/60, t%60)) #orinting the time in ther terminal
			...
			costText = self.font.render("Total Cost: $%02d"%(cost), True, (self.black), (self.white)) #calculate cost
			...
			#pygame sintax to fill in the screen 
			...
			for j in range(0, len(carRects)):
				...
				if len(path)-1 == counter: # if a car reaches its customer's destination, idle for 10 mins
					...
					carList[j] = car
					delay[j] = delay[j] + 1
					if delay[j] < 600: #10 minute delay
						continue
